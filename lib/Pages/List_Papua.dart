...
class List_Papua extends StatefulWidget {
  @override
  _List_PapuaState createState() => _List_PapuaState();
}

class _List_PapuaState extends State<List_Papua> {
  Map data;
  List userData;
  List list;
...
//  }
  Future getData() async {
    http.Response response = await http.get(
        "http://127.0.0.1:8080/apibdaya/api/papua"); //"http://lifei.id/api/papua/transaksi");
    data = json.decode(response.body);
    setState(() {
      userData = data["data"];
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          title: BoldText("Papua", 25, kblack),
          centerTitle: true,
          elevation: 0.0,
        ),
