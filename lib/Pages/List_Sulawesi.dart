...
class List_Sulawesi extends StatefulWidget {
  @override
  _List_SulawesiState createState() => _List_SulawesiState();
}

class _List_SulawesiState extends State<List_Sulawesi> {
  Map data;
  List userData;
  List list;
...
//  }
  Future getData() async {
    http.Response response = await http.get(
        "http://127.0.0.1:8080/apibdaya/api/sulawesi"); //"http://lifei.id/api/sulawesi/transaksi");
    data = json.decode(response.body);
    setState(() {
      userData = data["data"];
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          title: BoldText("Sulawesi", 25, kblack),
          centerTitle: true,
          elevation: 0.0,
        ),
