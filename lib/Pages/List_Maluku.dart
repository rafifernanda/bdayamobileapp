...
class List_Maluku extends StatefulWidget {
  @override
  _List_MalukuState createState() => _List_MalukuState();
}

class _List_MalukuState extends State<List_Maluku> {
  Map data;
  List userData;
  List list;
...
//  }
  Future getData() async {
    http.Response response = await http.get(
        "http://127.0.0.1:8080/apibdaya/api/maluku"); //"http://lifei.id/api/maluku/transaksi");
    data = json.decode(response.body);
    setState(() {
      userData = data["data"];
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          title: BoldText("Maluku", 25, kblack),
          centerTitle: true,
          elevation: 0.0,
        ),
