...
class List_Nusa extends StatefulWidget {
  @override
  _List_NusaState createState() => _List_NusaState();
}

class _List_NusaState extends State<List_Nusa> {
  Map data;
  List userData;
  List list;
...
//  }
  Future getData() async {
    http.Response response = await http.get(
        "http://127.0.0.1:8080/apibdaya/api/nusa"); //"http://lifei.id/api/nusa/transaksi");
    data = json.decode(response.body);
    setState(() {
      userData = data["data"];
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          title: BoldText("Nusa Tenggara", 25, kblack),
          centerTitle: true,
          elevation: 0.0,
        ),
