...
class List_Sumatra extends StatefulWidget {
  @override
  _List_SumatraState createState() => _List_SumatraState();
}

class _List_SumatraState extends State<List_Sumatra> {
  Map data;
  List userData;
  List list;
...
//  }
  Future getData() async {
    http.Response response = await http.get(
        "http://127.0.0.1:8080/apibdaya/api/sumatera");//"http://lifei.id/api/sumatra/transaksi");
    data = json.decode(response.body);
    setState(() {
      userData = data["data"];
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          title: BoldText("Sumatra", 25, kblack),
          centerTitle: true,
          elevation: 0.0,
        ),
